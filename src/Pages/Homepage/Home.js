async function loadComponent(targetSelector, url) {
  ...
  try {
    ...
    console.warn("loadComponent error", url, err);
  }
}

loadComponent("#app-header", "/src/Components/page-header/header.html");
...
async function ensureProducts(){ if (!ALL_PRODUCTS.length) await loadProducts(); }

function installCartIcons(){
  const cards = document.querySelectorAll(".product-card");
  cards.forEach(card => {
    if (!card.closest('#top-hot-grid')) return;
    if (card.querySelector('.cart2-icon')) return;
...
async function openDrinksModal(slug){
  ...
  if (document.querySelector(".drinks-swiper") && window.Swiper){
    drinksSwiper = new Swiper(".drinks-swiper", {
      ...
      spaceBetween: 32,
      loop: true,
      navigation: { nextEl: ".drinks-right .swiper-button-next", prevEl: ".drinks-right .swiper-button-prev" },
      breakpoints: { 1024:{slidesPerView:3}, 768:{slidesPerView:3}, 480:{slidesPerView:2}, 0:{slidesPerView:1} }
    });
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".js-open-drinks").forEach(el => el.addEventListener("click", () => openDrinksModal(el.dataset.cat)));
